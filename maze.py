...
#DO NOT TOUCH THIS, SIMPLY DRAWS THE MAZE AND DEFINES AGENT MOVES
...
#THIS FUNCTION HAS A BUG WHEN YOU TRY TO MOVE BACKWARDS IN THE CIRLE AND
#IF THE AGENT GOES > 50 CIRLE LEVEL AND THERE IS NO LINE DRAWN AT THIS LOCATION IT SIMPLY RETURNS TRUE
#INSTEAD OF FALSE EVEN THOUGH NO MOVEMENT OCCURED. SEE COMMENTS IN THE FUNCTION FOR THE SMALL CHANGE!
def Backward(agent):
    global circle_level
    if circle_level > 50 and circle_level < SUCCESS_LEVEL:
        ...
        if isBlocked(Line(p1,p2)):
            ...
            agent.right(-90)
            return False
    else:              #Added this else statement just to make sure it goes false if
        return False   #the agent enters an invalid position through going backwards
    return True
